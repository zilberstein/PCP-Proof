...
\begin{itemize}
\item Property 1: NP $\subseteq$ PCP$_{1-\epsilon}[O(\log n), 2, 4]$
\item Property 2: For any language $L\in$ NP there is a polynomial time transformation $T$ from instances of $L$ to a constraint graphs on 16 colors $\mathcal G_{16}$ such that if $x\in L$ then $\mathcal G_{16}$ is satisfiable and if $x\notin L$ then at most $(1-\epsilon)|E|$ of the constraints are simultaneously satisfiable
\end{itemize}
\item It will suffice to prove Property 2
