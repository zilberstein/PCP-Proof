...
\end{itemize}
Note this implies Property 1 from the previous slide, so it suffices to show that this implies NP $\subseteq$ PCP$_{1-\epsilon}[O(\log n), 2, 4]$
\end{block}
}

\frame{
\frametitle{Proof:}
Fix an input $G \in CG$. The proof oracle is $\Pi : V \mapsto \{0,1\}^4$. Assume WLOG $|E| = 2^k$ for some $k$. The verifier tosses it's coins and picks a random edge $(u, v) \in E$ (with edge constraint $c_{(u, v)}$). The verifier then reads $\Pi(u)$ and $\Pi(v)$ and accepts if and only if $c_{(u, v)}(\Pi(u), \Pi(v)) = 1$.
}
...
\begin{align*}
  ...
  \vdots&\\
  \text{UNSAT}(G^{\log n}) \ge&\; \min\{2^{\log n}\text{UNSAT}(G), \alpha\}
\end{align*}
\end{itemize}
}
...
\end{block}
Main Lemma implies Gap Amplification Lemma
